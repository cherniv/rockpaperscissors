...
export default class Cell extends Component {
  constructor(props) {
    ...
    this.state = ({
      ...
      nowhereToMove: props.nowhereToMove,
    })
  ...
  runFadeOutAnimation() {
    ...
    Animated.timing(                            
      this.fadeOutAnim,                      
      {
        duration: this.props.attackedAnimationDuration || fadeOutDuration,
        toValue: 0.0,
  ...
  componentWillReceiveProps(props) {
    ...
    if (this.state.moveTo != props.moveTo) {
      if (props.moveTo != null) {
        ...
        this.animate(isCorrect, isCorrect ? this.correctMovementAnimationEnded : this.incorrectMovementAnimationEnded );
      }
      this.setState({moveTo: props.moveTo, moveCorrect: isCorrect})
    }

    if (this.state.nowhereToMove != props.nowhereToMove) {
      if (props.nowhereToMove) {
        this.animate( false,  this.nowheretomoveMovementAnimationEnded );
      }
      this.setState({nowhereToMove: props.nowhereToMove})
    }
  }
  ...
  _handlePanResponderEnd(e, e2) {
    ...
    if ((dx <= 0 && dy <= 0 && dx >= dy) || (dx >= 0 && dy <=0 && absdy >= dx)) {
      dir = 0;
    } else
    if ((dx >= 0 && dy <= 0 && dx >= absdy) || (dx >= 0 && dy >= 0 && dy <= dx)) {
      dir = 1;
    } else
  ...
  nowheretomoveMovementAnimationEnded = () => {
    this.props.nowheretomoveAnimationFinished(this.props.id);
  }

  animate(isCorrect, completeCallback) {
    var settings = {
      duration: this.props.attackingAnimationDuration || attackDuration,
